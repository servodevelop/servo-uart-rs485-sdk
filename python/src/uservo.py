...
class UartServoManager:
	...
	def stop_on_control_mode(self, servo_id, method, power):
		...
		'''
		if self.is_debug:
			logging.info('停止指令 id={} method={} power={} mW'.format(servo_id, method, power))		
        # 打包请求数据
	...
	def update(self, is_empty_buffer=False, wait_response=False, timeout=0.005):
		'''舵机管理器的定时器回调函数'''
		t_start_ns = time.monotonic_ns()
		idle_threshold_ns = int(200_000)
		...
		while True:
			...
			if bytes_waiting:
				...
				if wait_response and (now_ns - last_data_time_ns) > idle_threshold_ns:
					break
				if not wait_response:
					break
				if (now_ns - t_start_ns) > timeout_ns:
					break
